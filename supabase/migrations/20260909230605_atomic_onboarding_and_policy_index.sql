create index cp_policy_acceptances_document
  on public.cp_policy_acceptances(policy_code,version);

-- A signed-in user may create exactly their own profile. They cannot update role afterwards
-- because authenticated still has no UPDATE grant on cp_profiles.
grant insert(id,role,display_name,specialty) on public.cp_profiles to authenticated;

create policy cp_profile_self_insert
on public.cp_profiles for insert to authenticated
with check (
  id=(select auth.uid())
...
begin
  ...
  if not exists(
    ...
    raise exception 'Debes confirmar tu correo antes de completar el perfil.' using errcode='42501';
  end if;
